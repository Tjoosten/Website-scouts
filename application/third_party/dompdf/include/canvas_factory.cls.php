...
class Canvas_Factory
{
  ...
  private function __construct()
  {
    ...
    public static function get_instance($paper = null, $orientation = null, $class = null)
    {
        $backend = strtolower(DOMPDF_PDF_BACKEND);

        if (isset($class) && class_exists($class, false)) {
            $class .= '_Adapter';
        } elseif ((DOMPDF_PDF_BACKEND === 'auto' || $backend === 'pdflib') &&
              class_exists('PDFLib', false)) {
            $class = 'PDFLib_Adapter';
        } elseif ((DOMPDF_PDF_BACKEND === 'auto' || $backend === 'cpdf')) {
            $class = 'CPDF_Adapter';
