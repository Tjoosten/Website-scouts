...
class Image_Cache
{
   ...
   */
  public static function resolve_url($url, $proto, $host, $base_path)
  {
    ...
    if ($DEBUGPNG) {
      ...
      } elseif (DOMPDF_ENABLE_REMOTE && $remote || $datauri) {
      ...
      $full_url = build_url($proto, $host, $base_path, $url);
        ...
        $imagedim = dompdf_getimagesize($resolved_url);
            ...
            in_array($imagedim[2], [IMAGETYPE_GIF, IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_BMP])) {
                      //target image is valid

        $imagetypes = ['','gif','jpeg','png','swf','psd','bmp'];
        ...
        self::$_cache[$full_url] = [$resolved_url,$ext];
                  } else {
