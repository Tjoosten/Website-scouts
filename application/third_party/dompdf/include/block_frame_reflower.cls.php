...
class Block_Frame_Reflower extends Frame_Reflower
{
  ...
  protected $_frame;
    ...
    public function __construct(Block_Frame_Decorator $frame)
    {
  ...
  public function vertical_align()
  {
    ...
    $top = $style->length_in_pt([$style->margin_top,
                                      $style->padding_top,
                                      $style->border_top_width, ], $cb['h']);

        $bottom = $style->length_in_pt([$style->border_bottom_width,
                                         $style->margin_bottom,
                                         $style->padding_bottom, ], $cb['h']);

        $cb_x = $x + $left_margin + $style->length_in_pt([$style->border_left_width,
    ...
    foreach ($this->_frame->get_children() as $child) {
      ...
      if (DOMPDF_ENABLE_CSS_FLOAT && count($floating_children)) {
        ...
        $child->set_containing_block($cb_x, $cb_y, $w, $cb_h);
          ...
          if ($offset_left) {
              $this->_frame->set_current_line(['left' => $offset_left]);
          }

          if ($offset_right) {
              $this->_frame->set_current_line(['right' => $offset_right]);
          }
      }
