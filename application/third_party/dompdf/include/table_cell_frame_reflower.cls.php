...
class Table_Cell_Frame_Reflower extends Block_Frame_Reflower
{
  ...
  public function reflow(Frame_Decorator $block = null)
  {
    ...
    $h = $this->_frame->get_containing_block('h');
      ...
      $left_space = $style->length_in_pt([$style->margin_left,
                                             $style->padding_left,
                                             $style->border_left_width, ],
                                       $w);

      $right_space = $style->length_in_pt([$style->padding_right,
                                              $style->margin_right,
                                              $style->border_right_width, ],
                                        $w);

      $top_space = $style->length_in_pt([$style->margin_top,
                                            $style->padding_top,
                                            $style->border_top_width, ],
                                      $h);
      $bottom_space = $style->length_in_pt([$style->margin_bottom,
