...
class  Cpdf
{
  ...
  /**
   * @var bool whether the compression is possible
   */
   ...
   */
  protected static $targetEncoding = 'iso-8859-1';
   ...
   * @var bool whether text will be treated as Unicode or not.
   */
...
//      }
  ...
  protected function o_info($id, $action, $options = '')
  {
    ...
    case  'new':
      $this->infoObject = $id;
      $date = 'D:'.@date('Ymd');
      $this->objects[$id] = ['t' => 'info', 'info' => ['Creator' => 'R and OS php pdf writer, http://www.ros.co.nz', 'CreationDate' => $date]];
      break;
    case  'Title':
    case  'Author':
    case  'Subject':
  ...
  protected function o_contents($id, $action, $options = '')
  {
    ...
    protected function o_embedjs($id, $action, $code = '')
    {
        ...
        switch ($action) {
    case  'new':
      $this->objects[$id] = ['t' => 'embedjs', 'info' => [
    ...
    protected function o_javascript($id, $action, $code = '')
    {
        ...
        switch ($action) {
    case  'new':
      $this->objects[$id] = ['t' => 'javascript', 'info' => [
        'S'                      => '/JavaScript',
  ...
  {
      static  $valid_params = ['LW',  'LC',  'LC',  'LJ',  'ML',
                                   'D',  'RI',  'OP',  'op',  'OPM',
                                   'Font',  'BG',  'BG2',  'UCR',
                                   'TR',  'TR2',  'HT',  'FL',
                                   'SM',  'SA',  'BM',  'SMask',
   ...
   */
  public function newDocument($pageSize = [0, 0, 612, 792])
  {
      $this->numObj = 0;
      $this->objects = [];
   ...
   */
  public function selectFont($fontName, $encoding = '', $set = true)
  {
   ...
   */
  public function setColor($color, $force = false)
  {
   ...
   */
  public function setStrokeColor($color, $force = false)
  {
      $new_color = [$color[0], $color[1], $color[2], isset($color[3]) ? $color[3] : null];

      if (!$force && $this->currentStrokeColour == $new_color) {
          return;
      }
   ...
   */
  public function setGraphicsState($parameters)
  {
   ...
   */
  public function setLineTransparency($mode, $opacity)
  {
      static $blend_modes = ['Normal', 'Multiply', 'Screen',
   ...
   * @param float $opacity 0.0 fully transparent, 1.0 fully opaque
   */
  public function setFillTransparency($mode, $opacity)
  {
      static $blend_modes = ['Normal', 'Multiply', 'Screen',
                                'Overlay', 'Darken', 'Lighten',
                                'ColorDogde', 'ColorBurn', 'HardLight',
   ...
   */
  public function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart = 0, $afinish = 360, $close = true, $fill = false)
  {
      if ($r1 ==  0) {
          return;
      }
   ...
   */
  public function polygon($p, $np, $f = false)
  {
      $this->objects[$this->currentContents]['c'] .=  sprintf("\n%.3F %.3F m ", $p[0], $p[1]);
   ...
   */
  public function clippingRectangle($x1, $y1, $width, $height)
  {
      $this->save();
   ...
   */
  public function filterText($text, $bom = true, $convert_encoding = true)
  {
      if (!$this->numFonts) {
          $this->selectFont($this->defaultFont);
      }
   ...
   */
  public function utf8toCodePointsArray(&$text)
  {
   ...
   *
   * @param string $text UTF-8 string to process
   * @param bool $bom whether to add the byte order marker
   *
   * @return string UTF-16 result string
   */
  public function utf8toUtf16BE(&$text, $bom = true)
  {
      $cf = $this->currentFont;
            ...
            $tmp = mb_substr($text, $j + 1, $k - $j - 1);
              ...
              if ($final) {
                ...
                $tmp = $this->PRVTgetTextPosition($x, $y, $angle, $size, $wordSpaceAdjust, mb_substr($text, 0, $i));
                  ...
                  $info = ['x' => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'end', 'p' => $parm, 'nCallback' => $this->nCallback];
                  $x = $tmp[0];
                  $y = $tmp[1];
                  $ret = $this->$func($info);
   ...
   */
  public function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0, $charSpaceAdjust = 0, $smallCaps = false)
  {
      if (!$this->numFonts) {
          $this->selectFont($this->defaultFont);
      }
      ...
      if ($smallCaps) {
          $text = preg_replace_callback("/\p{Ll}/u", [$this, 'toUpper'], $text);
      }
   ...
   */
  public function getTextWidth($size, $text, $word_spacing = 0, $char_spacing = 0)
  {
   ...
   */
  public function serializeObject($id)
  {
   ...
   */
  public function restoreSerializedObject($obj)
  {
   ...
   */
  public function addImagePng($file, $x, $y, $w = 0, $h = 0, &$img, $is_mask = false, $mask = null)
  {
    ...
    for ($xpx = 0; $xpx < $wpx; ++$xpx) {
        for ($ypx = 0; $ypx < $hpx; ++$ypx) {
          ...
          $pixel = (127 - $alpha) * 2;
                ...
                $key = implode('-', [$col['red'], $col['green'], $col['blue']]);
   ...
   */
  public function addPngFromBuf($file, $x, $y, $w = 0, $h = 0, &$data, $is_mask = false, $mask = null)
  {
      if (isset($this->imagelist[$file])) {
          //debugpng
   ...
   */
  public function image_iscached($imgname)
  {
   ...
   */
  public function addJpegImage_common(&$data, $x, $y, $w = 0, $h = 0, $imageWidth, $imageHeight, $channels = 3, $imgname)
  {
